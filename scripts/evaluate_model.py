...
def main():
    ...
    print(
        f"Métricas obtenidas: Accuracy={accuracy:.2f}, Precision={precision:.2f}, Recall={recall:.2f}"
    )

    # Validar que todos superen el 80% (0.80)
    if accuracy < 0.80 or precision < 0.80 or recall < 0.80:
        print(
            "❌ ERROR: Una o más métricas cayeron por debajo del 80%. El modelo es inválido."
        )
